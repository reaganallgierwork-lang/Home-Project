...
function stateWith(sessions) {
  return { settings: { ...DEFAULTS }, habits: [], log: {}, seen: [], ui: {}, exercises, templates: [], sessions };
}
...
console.log('\n4. AMRAP — rounds plus the partial round');
{
  ...
  ok('an unlogged AMRAP records nothing', W.blockRecords(b).length === 0);
}
...
console.log('\n5. Rounds for time — credit what was actually finished');
{
  ...
  ok('finishing all rounds credits them all', W.blockRecords(b)[0].reps === 50);

  b.result = { seconds: 1200, roundsDone: 3, capped: true };
  ok('capping out credits only the rounds completed', W.blockRecords(b)[0].reps === 30);

  b.result = { seconds: 700, roundsDone: null, capped: false };
  ok('a time with no round count assumes the workout was completed', W.blockRecords(b)[0].reps === 50);
}
